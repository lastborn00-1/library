    ...
    <div class="relative z-10 w-full max-w-md mx-auto px-4 sm:px-6">
        <div class="bg-white/95 backdrop-blur-md p-8 sm:p-10 rounded-2xl shadow-2xl border border-slate-100">
            ...
            <div class="text-center mb-6">
                <h2 class="text-2xl font-black text-slate-900">Welcome to KWCHT E-Library</h2>
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div x-data="{ show: false }">
                    ...
                    <div class="relative">
                        <input id="password" :type="show ? 'text' : 'password'" name="password" required autocomplete="current-password"
                               class="block w-full px-4 py-2.5 rounded-xl border border-slate-300 bg-white text-slate-900 focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 transition shadow-sm sm:text-sm pr-10"
                               placeholder="••••••••">
                ...
                <div class="flex items-center justify-between">
                    <label for="remember_me" class="inline-flex items-center">
                        <input id="remember_me" type="checkbox" class="rounded border-slate-300 text-emerald-600 shadow-sm focus:ring-emerald-600 cursor-pointer" name="remember">
                        <span class="ms-2 text-sm text-slate-600">{{ __('Remember me') }}</span>
                    </label>

                    @if (Route::has('password.request'))
                        <a class="text-sm font-semibold text-emerald-700 hover:text-emerald-800 hover:underline" href="{{ route('password.request') }}">
                            {{ __('Forgot password?') }}
                        </a>
                    @endif
                </div>
                ...
                <div class="relative my-4">
                    ...
                    <div class="relative flex justify-center text-xs"><span class="px-3 bg-white text-slate-400 font-medium">New student?</span></div>
                </div>
                ...
                @if($regEnabled)
                    <a href="{{ route('student.register') }}"
                       class="w-full flex justify-center py-2.5 px-4 border-2 border-emerald-200 rounded-xl text-sm font-bold text-emerald-700 hover:bg-emerald-50 hover:border-emerald-400 transition">
