<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-semibold text-xl text-slate-800 leading-tight">Edit Department: {{ $department->name }}</h2>
        </div>
    </x-slot>
                ...
                <form action="{{ route('departments.update', $department) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3"
                                  class="w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $department->description) }}</textarea>
