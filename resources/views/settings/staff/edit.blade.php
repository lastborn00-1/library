    ...
    <div class="py-10">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden p-8">
                <form action="{{ route('settings.staff.update', $staff) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title', $staff->title) }}" required
                                class="w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition">
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-4 mb-2">
                            @if($staff->image_path)
                                <img src="{{ asset('storage/' . $staff->image_path) }}" class="w-16 h-16 rounded-full object-cover border border-slate-200 shadow-sm">
                            @endif
